...
- ✅支持自定义阶梯电价（3 档 / 按年累计），并本地计算当前档位、档位电价、本档剩余电量与本年预估电费
...
- 本地阶梯数据（按自定义阶梯本地计算：档位、档位电价、本档剩余电量、本年预估电费，需启用）
- 昨日用电量
...
> 📌 **本分支新增功能**：支持自定义阶梯电价（3 档 / 按年累计），并据此**本地计算**当前档位、档位电价、本档剩余电量与本年预估电费。配置方法与默认值见下方[阶梯电价设置（本地计算）](#阶梯电价设置本地计算)。

❌仍**不支持**峰谷电价设置；除上述阶梯电价外不进行其他电费计算。南网暂时没有统一的电价 API，阶梯计算完全基于你手动填写的参数。
...
南网接口虽然会下发“当前阶梯”，但偶尔为空、且不含完整的阶梯电量/电价表。本集成支持在 **选项 → 阶梯电价设置** 中手动配置一套 **3 档、按年累计** 的阶梯电价，并基于“今年累计用电量”在本地计算，新增以下传感器：

| 传感器 | 说明 |
|---|---|
| 本地阶梯档位 | 当前年累计用电量所处的档位（1 / 2 / 3）|
| 本地阶梯电价 | 当前档位对应单价（元/kWh）|
| 本地阶梯剩余电量 | 距下一档阈值还剩多少 kWh（顶档无上限，显示为不可用）|
| 本年阶梯预估电费 | 年累计用电量按各档分段计价之和（元）|

配置项：

- 启用本地阶梯电价计算（开关）
- 第 1 / 2 档 年累计电量上限（kWh）
- 第 1 / 2 / 3 档 电价（元/kWh，第 3 档无上限）

保存时会校验“第 2 档上限必须大于第 1 档”；保存后自动重载集成使配置生效，并按启用状态创建/移除上述传感器。

**默认阶梯**（可在设置中按本地实际修改）：

| 档位 | 年累计电量 | 电价（元/kWh）|
|---|---|---|
| 第 1 档 | ≤ 2760 kWh | 0.5283 |
| 第 2 档 | 2760 ~ 4440 kWh（含 4440）| 0.5783 |
| 第 3 档 | > 4440 kWh | 0.8283 |

计费方式为**分段累进**。例如年累计 5000 kWh 时：

```
预估电费 = 2760 × 0.5283 + (4440 − 2760) × 0.5783 + (5000 − 4440) × 0.8283
        = 2893.5 元
```

> 说明：本地计算依赖“今年累计用电量”传感器，需在首次成功取到年累计数据后才会有值，取不到时显示为不可用。该功能完全基于你填写的参数，与南网下发的“当前阶梯”系列传感器相互独立、并存。
